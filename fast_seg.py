...
class FastSeg:
    ...
    def run_seg_only(self, in_f, prefix, batch_size=1, no_cereb=False):
        ...
        if proc.wait() != 0:
            ...
            print(res[0].decode())
            print(res[1].decode())
            assert subprocess.CalledProcessError
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
                    prog='fast_seg',
                    description='Fast brain segmentation using Fast Surfer',
                    epilog='mmisaki@libr.net')
    parser.add_argument('input', help='input anatomy image')
    parser.add_argument('-o', '--outprefix')
    parser.add_argument('-s', '--seg', nargs='+',
